...
func (u *PodDataService) CreateToK8s(podInfo *pod.PodInfo) (err error) {
	...
	if _, err = u.K8sClientSet.AppsV1().Deployments(podInfo.PodNamespace).Get(context.TODO(), podInfo.PodName, v12.GetOptions{}); err != nil {
		...
		// 可以写自己的业务逻辑
		errMsg := fmt.Sprintf("Pod %s 已经存在", podInfo.PodName)
		common.Error(errMsg)
...
func (u *PodDataService) SetDeployment(podInfo *pod.PodInfo) {
	deploymemt := &v1.Deployment{}
	deploymemt.TypeMeta = v12.TypeMeta{
		Kind:       "deployment",
		APIVersion: "v1",
	}

	deploymemt.ObjectMeta = v12.ObjectMeta{
	...
	deploymemt.Name = podInfo.PodName
	deploymemt.Spec = v1.DeploymentSpec{
		// 副本个数
